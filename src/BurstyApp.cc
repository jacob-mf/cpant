...
void BurstyApp::generatePacket()
{
    ...
    // place to generate kind of packet 0: Conversational 1: Streaming 2: Web 3: Data
    // 4 : Forward Ant 5 : Backward ant
    Packet *pk = new Packet(pkname);
    pk->setByteLength(packetLengthBytes->longValue());
    pk->setSrcAddr(myAddress);
    pk->setDestAddr(destAddress);
    pk->setTransientNodesArraySize(99); // initialize to safe value
    send(pk,"out");
}
